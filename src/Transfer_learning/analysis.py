...
gene_zero_one_weight = gene_nor_weight / np.sum(gene_nor_weight, axis=0).reshape(1, 32)
...
for i, item in enumerate(sorted_cancer_gene):
    ...
    for gene in item:
        ...
        if cancer_prob[cfg.ORGAN_NAMES[i]][cfg.GENE_NAMES[:32][gene[0]]].values[0] > 0.006 and gene[1] > 0:
            genes.append(cfg.GENE_NAMES[:32][gene[0]])
